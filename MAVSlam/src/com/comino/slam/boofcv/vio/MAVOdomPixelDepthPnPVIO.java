/*
 * Copyright (c) 2011-2015, Peter Abeles, Eike Mansfeld. All Rights Reserved.
 *
...
/**
 * Full 6-DOF visual odometry where a ranging device is assumed for pixels in
 * the primary view and the motion is estimated using a
 * {@link boofcv.abst.geo.Estimate1ofPnP}. Range is usually estimated using
 * stereo cameras, structured light or time of flight sensors. New features are
 * added and removed as needed. Features are removed if they are not part of the
 * inlier feature set for some number of consecutive frames. New features are
 * detected and added if the inlier set falls below a threshold or every turn.
 *
 * Non-linear refinement is optional and appears to provide a very modest
 * improvement in performance. It is recommended that motion is estimated using
 * a P3P algorithm, which is the minimal case. Adding features every frame can
 * be computationally expensive, but having too few features being tracked will
 * degrade accuracy. The algorithm was designed to minimize magic numbers and to
 * be insensitive to small changes in their values.
 *
 * Due to the level of abstraction, it can't take full advantage of the sensors
 ...
 */
public class MAVOdomPixelDepthPnPVIO<T extends ImageBase>  {

	// when the inlier set is less than this number new features are detected
	private int thresholdAdd;

	// discard tracks after they have not been in the inlier set for this many
	// updates in a row
	...
	// estimate the camera motion up to a scale factor from two sets of point
	// correspondences
	 ...
	 *
	 * @param thresholdAdd
	 *            Add new tracks when less than this number are in the inlier
	 *            set. Tracker dependent. Set to a value &le; 0 to add features
	 *            every frame.
	 * @param thresholdRetire
	 *            Discard a track if it is not in the inlier set after this many
	 *            updates. Try 2
	 * @param doublePass
	 *            Associate image features a second time using the estimated
	 *            model from the first try to improve results
	 * @param motionEstimator
	 *            PnP motion estimator. P3P algorithm is recommended/
	 * @param pixelTo3D
	 *            Computes the 3D location of pixels.
	 * @param refine
	 *            Optional algorithm for refining the pose estimate. Can be
	 *            null.
	 * @param tracker
	 *            Point feature tracker.
	 * @param pixelToNorm
	 *            Converts from raw image pixels into normalized image
	 *            coordinates.
	 * @param normToPixel
